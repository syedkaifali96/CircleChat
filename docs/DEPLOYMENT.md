...
- **Local media storage without R2:** run local MinIO (`minio.exe server .\data --address :9000 --console-address :9001`, any S3 credentials) and set `R2_ENDPOINT=http://127.0.0.1:9000` in `.env` — the storage gateway (standard AWS SDK `S3Client`) then uses MinIO with path-style signing and media features work fully against the local `circlechat-dev` bucket. Unset `R2_ENDPOINT` targets real Cloudflare R2.
- **USB debugging:** with an Android device connected over USB, `adb reverse tcp:8081 tcp:8081 && adb reverse tcp:3000 tcp:3000` lets the phone reach Metro and the local API server over USB via `localhost` — no WiFi/IP configuration needed (the app's default `EXPO_PUBLIC_API_URL` fallback of `http://localhost:3000` matches this). With MinIO running, add `adb reverse tcp:9000 tcp:9000` so presigned upload/download URLs also resolve from the phone.
