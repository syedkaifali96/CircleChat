...
- **Create/Join Circle screens had no header:** both screens now render the app's standard header row (title + back chevron, matching the Chats/Home pattern) with working back navigation (`create-circle-header`/`join-circle-header`, `create-circle-back`/`join-circle-back`). The header container pads its top with the safe-area inset — see the status-bar clipping entry below for the provider fix that made those insets real.
- **Fixed: Create Circle and Join Circle headers were clipped by the status bar; now use safe-area top inset.** The earlier pass padded them with `max(insets.top, 24) + 12`, but the app never mounted `<SafeAreaProvider>`, so `useSafeInsets()` always fell back to zeros and the headers were in practice padded by the hardcoded 24 floor — not enough on devices with a taller status bar or a display cutout. The root layout now mounts `SafeAreaProvider` (`initialWindowMetrics`) and both header containers pad by the reported inset (`insets.top + 12`), so the title and back chevron clear the status bar on every device. Regression tests inject insets below the old floor (8 → 20) and above it (40 → 52) and assert the value flows into the container's `paddingTop`, plus a root-layout test that the provider is mounted.
